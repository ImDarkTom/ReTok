...
async function getNextVideo() {
    loopPos++;
    try {
        currentVideoData = currentVideos[loopPos].data;
        nextVideoData = currentVideos[loopPos + 1].data;

        loadVideo(currentVideoData, nextVideoData);

    } finally {
        if (loopPos >= currentVideos.length - 1 || currentVideos.length == 0) {
            const url = `https://www.reddit.com/${subreddit ? `r/${subreddit}/` : ''}search.json?q=${query ? `${query}` : ''} type:video&include_over_18=${settings.filterMature != null ? (settings.filterMature ? '1': '0'): '1'}&limit=25&after=${nextFullname}${subreddit ? '&restrict_sr=1' : ''}`;
